...
function BuildingHelper:PlaceBuilding(player, name, location, snapToGrid, blockGridNav, size)
  ...
  if snapToGrid then
	...
	print ("creating building:"..name..", with player hero:"..playersHero.GetName().." and team "..playersHero:GetTeamNumber())
  -- Spawn the building
...
function BuildingHelper:AddBuilding(keys)
  ...
  CustomGameEventManager:Send_ServerToPlayer(player, "building_helper_enable", {["state"] = "active", ["size"] = size/2} )
end
...
function BuildingHelper:InitializeBuildingEntity( keys )
  ...
  building.healthChecker = Timers:CreateTimer(.2, function()
   ...
   function building:getBuildingCost()
		return building.cost
   end
   function building:setBuildingCost(fCost)
		building.cost = fCost
   end
  function building:RemoveBuilding( bForcedKill )
    -- Thanks based T__
